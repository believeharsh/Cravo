...
const Navbar = ({
  ...
  cartCount = 0,
  // onCartClick, onProfileClick, onHelpClick, onOffersClick, onSearchClick,
  // onCorporateClick, onLogoClick - these props can be replaced by 'to' paths
}) => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isSearchFocused, setIsSearchFocused] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            >
              <div className="w-8 h-8 lg:w-10 lg:h-10 bg-yellow-400 rounded-full flex items-center justify-center shadow-md">
                <span className="text-gray-800 font-bold text-lg lg:text-xl">
                  🛒 {/* Consider replacing with the actual 'Cravo' C logo */}
                </span>
              </div>
              <h1 className="text-xl lg:text-2xl font-bold text-gray-800">
                Cravo
                  ...
                  <NavLink // <--- Use NavLink here
                    ...
                    className={({ isActive }) =>
                      `relative flex items-center space-x-2 px-4 py-2 rounded-xl font-medium transition-all duration-200 hover:bg-gray-50 hover:scale-105 ${
                        isActive // Use isActive directly
                          ? "bg-yellow-50 text-yellow-600"
                          : "text-gray-700 hover:text-gray-900"
                      }`
                    }
