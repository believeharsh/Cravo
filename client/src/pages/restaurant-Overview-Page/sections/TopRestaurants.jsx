...
const TopRestaurants = () => {
  ...
  useEffect(() => {
    // Determine the effective location context: URL takes precedence over Redux
    const urlCity = searchParams.get('cityName');
    ...
    const finalCity = urlCity || defaultCity;
    const finalLat = urlLat || defaultLat;
    const finalLng = urlLng || defaultLng;
      ...
      <section className="bg-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          <div className="bg-red-50 border border-red-200 rounded-lg p-8">
            ...
            {/* Note: Reload is usually fine, but for better UX, you could dispatch a thunk to re-fetch here if you weren't avoiding global state */}{' '}
            <button
              onClick={() => window.location.reload()}
              className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors"
            >
          ...
          >
            {restaurants.map(restaurant => {
              ...
              return (
                ...
                >
                  <RestaurantCard data={restaurant} className="max-h-96" />
                </Link>
              );
            })}
          </div>
